pipeline {
    ...
    environment {
        ...
        TOMCAT_URL = 'http://3.81.95.53/manager/text'
        TOMCAT_USER = 'admin'
        TOMCAT_PASS = 'admin'
    ...
    stages {
        stage('Clone Repository') {
            steps {
                git url: 'https://github.com/2300030871/fullstackapp.git', branch: 'master'
            }
        }
    ...
    post {
        success {
            echo "✅ Backend deployed: http://3.81.95.53/springapp1"
            echo "✅ Frontend deployed: http://3.81.95.53/frontapp1"
        }
        failure {
